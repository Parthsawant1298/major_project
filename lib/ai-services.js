...
export async function generatePersonalizedQuestions({ 
  ...
  try {
    const prompt = `
    Based on this candidate's resume and the job requirements, personalize the interview questions:
    
    Resume: ${resumeText.substring(0, 2000)}
    Job Title: ${jobTitle}
    Job Description: ${jobDescription}
    
    Base Questions: ${baseQuestions.map(q => q.question).join('\n')}
    
    Generate 3-5 personalized follow-up questions that:
    1. Address specific experiences mentioned in their resume
    2. Explore gaps or areas needing clarification  
    3. Test practical application of their stated skills
    
    Return ONLY a JSON array of questions in this format:
    [
      {
        "question": "I see you worked with React at XYZ company. Can you walk me through a challenging component you built?",
        "type": "technical",
        "difficulty": "medium",
        "expectedDuration": 180,
        "resumeBased": true
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert interviewer who creates personalized questions based on candidate backgrounds.'
        },
        {
          role: 'user',
          content: prompt
